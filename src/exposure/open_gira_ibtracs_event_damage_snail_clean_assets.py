"""
Snail event-by-event tropical cyclone damage analysis for cleaned Florida lines.
...
This clean-assets version uses the current cleaned electricity layer:

    data/Electricity/florida_lines_with_s_nom.gpkg

Run this with the open-gira Pixi/WSL environment because Snail is not installed
in the Windows Python environment:
    ...
    python data/Exposure/open_gira_ibtracs_event_damage_snail_clean_assets.py"

Outputs:
    data/Exposure/florida_clean_assets_tc_event_damage_snail/
"""
...
OUTPUT_DIR = PROJECT_DIR / "data" / "Exposure" / "florida_clean_assets_tc_event_damage_snail"
...
FLORIDA_LINES = ELECTRICITY_DIR / "florida_lines_with_s_nom.gpkg"
...
def load_florida_overhead_lines() -> gpd.GeoDataFrame:
    """Load cleaned Florida lines and keep overhead, in-service transmission lines."""
    ...
    if "voltage_kv" in lines.columns:
        lines["voltage_kv_raw"] = pd.to_numeric(lines["voltage_kv"], errors="coerce")
    else:
...
def make_graphs(event_damage: pd.DataFrame) -> None:
    ...
    ax.set_title("Snail clean-assets: Top 20 IBTrACS Events by Florida Transmission Line Damage")
    ...
    ax.set_title("Snail clean-assets: Distribution of Historical IBTrACS Event Damages")
    ...
    ax.set_title("Snail clean-assets: Historical Event Damage Exceedance Curve")
    ax.grid(alpha=0.25)
    plt.tight_layout()
...
def main() -> None:
    ...
    print(f"Loaded cleaned Florida overhead in-service lines: {len(lines):,}")
